"""GENERATION STEP (not part of reproduce.py). Scores the corpus with the language
model; requires the raw Wikipedia plot text (inputs/) and an OpenAI API key, neither
included in this release. The shipped scored corpus (data/corpus/, data/atlas/) is the
output, so reproduction reads the released data and does not re-run this script.
