...
from sklearn.linear_model import LogisticRegression; from sklearn.model_selection import cross_val_score; from sklearn.metrics import silhouette_score
...
# ===== ED3 era-predictability =====
try:
    PAL={"film":"#c0392b","book":"#2c3e50","tv":"#27ae60"}
    fig,ax=plt.subplots(figsize=(7,4.5)); res=[]
    for d,nm in [(F15,"film"),(B15,"book"),(T15,"tv")]:
        ...
        acc=cross_val_score(LogisticRegression(max_iter=300),StandardScaler().fit_transform(dd[ATTRS]),dd.dec,cv=3).mean()
        res.append((nm,acc,1/dd.dec.nunique()))
    ax.spines[["top","right"]].set_visible(False); ax.set_axisbelow(True); ax.grid(axis="y",alpha=0.15)
    ax.bar([r[0] for r in res],[r[1] for r in res],color=[PAL[r[0]] for r in res],width=0.62)
    for i,r in enumerate(res): ax.axhline(r[2],i/len(res),(i+1)/len(res),color="0.35",ls=":",lw=1.5,label=("chance" if i==0 else None))
    ax.legend(frameon=False,loc="upper left")
    ax.set_ylabel("decade-prediction accuracy"); ax.set_title("How era-stamped is each medium's style?",loc="left",fontweight="bold")
