...
# --------------------------------------------------------------- load data ----
df = pd.read_csv(CSV)
# The codebook also carries the expansion attributes (identity, narration, and
# element-importance additions) under the Headline/Validated/Marginal tier scheme; these
# are validated but not part of the deployed instrument. The shipped atlas is the 161
# attributes graded on the A/B/C tiers, so restrict the dictionary to those, which keeps
# the table and its counts consistent with the deployed instrument.
DEPLOYED_TIERS = {"A", "B", "C", "--"}
df = df[df["tier"].isin(DEPLOYED_TIERS)].reset_index(drop=True)
