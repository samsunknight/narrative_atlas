...
chk("R", "descriptor visual median r", 0.45, round(float(VIS.r.median()), 3), 0.03)
chk("R", "descriptor score median r",  0.41, round(float(DESC[DESC.attr_id.str.contains("score", case=False)].r.median()), 3), 0.03)
chk("R", "descriptor acting median r", 0.32, round(float(DESC[DESC.attr_id.str.contains("acting", case=False)].r.median()), 3), 0.03)
...
#   structure 40, mood 28 = shipped §11 sweep counts     (asserted; no per-attr table in data/)
# =====================================================================================
...
# not asserted -- so it can never silently drift from the codebook again. After the
# reader-fit drop the structure layer holds 46 attributes, 40 of which validate.
...
chk("R", "validated count: structure (codebook H+V)", 40, n_struct, 0)
chk("A", "validated count: mood (shipped §11q)",      28, 28, 0)
chk("R", "validated count: total (129)", 129, n_struct + 28 + n_genre_layer + n_arc_layer + n_tex_film, 0)
